...
class SortingHatApp(QMainWindow):
    ...
    def _init_ui_elements(self):
        ...
        # --- DYNAMIC STYLESHEET SECTION ---
        BG_COLOR = "#202123"
        INPUT_BG_COLOR = "#40414f"
        TEXT_COLOR = "#ececf1"
        
        # Get the font size from settings, with a safe default
        dialogue_font_size = self.settings_manager.get_setting("ui_settings.dialogue_font_size", 28)
        try:
            # Ensure it's a valid number
            dialogue_font_size = int(dialogue_font_size)
            if dialogue_font_size < 8: dialogue_font_size = 8 # Set a minimum
        except (ValueError, TypeError):
            dialogue_font_size = 28 # Fallback on error
            print(f"WARNING: Invalid 'dialogue_font_size' in settings. Falling back to {dialogue_font_size}pt.")

        # Build the stylesheet dynamically
        stylesheet = f"""
            ...
            QLabel {{ color: {TEXT_COLOR}; padding: 2px; }}
            QLabel#titleLabel {{
                font-size: 16pt;
                font-weight: bold;
                color: {TEXT_COLOR};
            ...
            QPushButton:hover, QToolButton:hover {{ background-color: #4a4b57; }}
            QPushButton:disabled, QToolButton:disabled {{
                background-color: #2a2b32;
                color: #6a6b70;
                border: 1px solid #40414f;
            }}
            /* This targets ONLY the dialogue boxes */
            QTextEdit {{
                ...
                border-radius: 5px;
                padding: 6px;
                font-size: {dialogue_font_size}pt;
            }}
            QComboBox {{
                background-color: {INPUT_BG_COLOR};
                border: 1px solid #565869;
            ...
            QComboBox QAbstractItemView {{
                ...
                border: 1px solid #565869;
                color: {TEXT_COLOR};
                selection-background-color: #4a4b57;
            }}
            QMessageBox {{ background-color: {INPUT_BG_COLOR}; font-size: 9pt; }}
            QMessageBox QLabel {{ color: {TEXT_COLOR}; }}
            QMessageBox QPushButton {{ background-color: #343541; min-width: 70px; padding: 6px 12px;}}
            ...
            QSlider::handle:horizontal:hover {{ background: #a9aaae; }}
        """
        self.setStyleSheet(stylesheet)
        # --- END OF DYNAMIC STYLESHEET SECTION ---
    ...
    def _reset_interaction_flow_and_ui(self, initial_message="Waiting for a lucky student..."):
        ...
        try:
            ...
            if min_q <= 0 or max_q <= 0:
                min_q, max_q = 1, 1
                print(f"WARNING: Min/max questions must be positive. Using final fallback range 1-1.")
            if min_q > max_q:
                max_q = min_q
